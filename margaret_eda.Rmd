...
transform_model = lm(target_death_rate ~ log_avg_ann_count + incidence_rate + inverse_med_income + sqrt_poverty_rate + 
     ...
     pct_married_households + sqrt_birth_rate,
   data = model_data)


#log_study_per_cap, log_pct_black, log_pct_asian,  log_other_race cause the X matrix to be singular
```
...
non_transform_model = lm(target_death_rate ~ log_avg_ann_count + incidence_rate + med_income + poverty_percent +
     ...
     pct_white + pct_married_households + birth_rate,
   data = model_data)
...
step(transform_model, direction = "both")

step(non_transform_model, direction = "both")
```
