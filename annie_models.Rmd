...
cv_df = 
  cv_df %>% 
  mutate(subset_trans = map(train, ~lm(target_death_rate ~ incidence_rate + pct_hs18_24 + log_pct_bach_deg25_over
                       + pct_married_households, data = .x)),
         rmse_test_subset_trans = map2_dbl(subset_trans, test, ~rmse(model = .x, data = .y)),
         rmse_train_subset_trans = map2_dbl(subset_trans, train, ~rmse(model = .x, data = .y)),
...
mean(cv_df$rmse_test_subset_trans)
mean(cv_df$rmse_train_subset_trans)
