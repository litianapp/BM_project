...
selected_transformed  = lm(target_death_rate ~ incidence_rate + log_avg_household_size + pct_hs18_24 +
                               pct_bach_deg25_over + pct_white + pct_married_households + 
                               log_study_per_cap + log_pct_black, data = transformed_model_data_state)

vif(selected_transformed)

```

There are no variables with VIF > 5. So, we arrive at a 8 predictor model with no collinearity issues: incidence_rate, pct_asian, birth_rate, med_income, avg_household_size, pct_other_race, percent_married, and pct_bach_deg25_over